...
(def usage
  ...
  `meg/match` using arguments contained in `file`.
  `meg/match`'s signature is the same as that of `peg/match`.
  `file` should be a jdn file with forms for each desired
  argument.  Files will be generated in the directory in
...
(defn main
  ...
  (when (opts :trace)
    (when (not (first rest))
      (eprintf "Failed to find argument file.")
      (os/exit 1))
    (def arg-file (first rest))
    (when (not (os/stat arg-file))
      (eprintf "Failed to find file: %s" arg-file)
      (os/exit 1))
    (def content (slurp arg-file))
